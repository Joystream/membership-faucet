...
export class JoyApi {
  ...
  constructor(endpoint?: string) {
    ...
    const wsEndpoint = endpoint || WS_PROVIDER || 'ws://127.0.0.1:9944'
  ...
  get init(): Promise<JoyApi> {
    return this.isReady.then((instance) => {
      ...
      this.signingPair = this.keyring.addFromUri(
        SCREENING_AUTHORITY_SEED,
        undefined,
        'sr25519'
      )
  ...
  makeGiftMembershipTx(memberData: NewMember) {
    const { account, handle, about, name, avatar, externalResources } =
      memberData
    ...
    return new Promise(async (resolve, reject) => {
      const callback: Callback<ISubmittableResult> = (result) => {
        ...
        if (result.isError) {
          ...
          const { isDropped, isFinalityTimeout, isInvalid, isUsurped } =
            result.status
          return reject(
